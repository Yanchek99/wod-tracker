...
## CrossFit.com WOD Fetcher & Parser
- Fetch and parse a single day's WOD from crossfit.com and print the result (does not persist anything):
  ```
...
- The printed `CfWod::ParseResult#status` is `:parsed` (fully structured, or an already-seeded named workout found directly), `:partial` (built something, but `#reason` names a gap), or `:failed` (nothing usable, including rest days).
