module CfWorkout
  class WorkoutParser
    ...
    def self.call(workout_page) = new(workout_page).parse

    def initialize(workout_page)
      @workout_page = workout_page
    end

    def parse
      lines = workout_page.body_text.split("\n")
      find_named_workout(lines) || parse_from_prose(lines)
    end

    private

    attr_reader :workout_page

    def parse_from_prose(lines)
      header, body = extract_header_and_body(lines)
      attrs = WorkoutFormatClassifier.call(header)
      workout = Workout.new(name: "CF-#{workout_page.slug}", **attrs.except(:lift_name, :set_reps))
