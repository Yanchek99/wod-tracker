...
fight_gone_bad = Workout.find_or_create_by(name: 'Fight Gone Bad') do |workout|
  ...
  workout.notes = 'In this workout you move from each of 5 stations after a minute.'\
          ...
          ' On the call of "rotate," the athlete(s) must move to the next station immediately for a good score.'\
          ' One point is given for each rep, except on the rower where each calorie is 1 point.'
  workout.exercises.build(movement: wallball, reps: 0, seconds: 60)
  workout.exercises.build(movement: sumo_deadlift_hight_pull, reps: 0, weight: 75, seconds: 60)
  workout.exercises.build(movement: box_jump, reps: 0, seconds: 60)
  workout.exercises.build(movement: push_press, reps: 0, weight: 75, seconds: 60)
  workout.exercises.build(movement: row, calories: 0, seconds: 60)
  workout.exercises.build(movement: rest, reps: 1, seconds: 60)
end
