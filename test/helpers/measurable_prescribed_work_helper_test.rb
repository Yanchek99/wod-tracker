...
class MeasurableLeadingPrescriptionIntegrationTest < ActionView::TestCase
  ...
  test 'renders 1600 meter Run without reps' do
    exercise = workouts(:fran).segments.first.exercises.create!(movement: movements(:run), position: 3,
  ...
  test 'renders 20 calorie Row' do
    exercise = workouts(:fran).segments.first.exercises.create!(movement: movements(:row), position: 3, calories: 20)
  ...
  test 'renders 80ft Dumbbell Overhead Walking Lunge (♀35lb / ♂50lb)' do
    ...
    exercise = workouts(:fran).segments.first.exercises.create!(movement: lunge, position: 3,
