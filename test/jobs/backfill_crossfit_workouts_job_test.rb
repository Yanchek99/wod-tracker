...
class BackfillCrossfitWorkoutsJobTest < ActiveJob::TestCase
  test 'enqueues one ScrapeCfWorkoutJob per date in the range, staggered by DELAY_BETWEEN_REQUESTS' do
    travel_to Time.utc(2026, 1, 1, 12, 0, 0) do
      BackfillCrossfitWorkoutsJob.perform_now(Date.new(2026, 1, 1), Date.new(2026, 1, 3))

      [Date.new(2026, 1, 1), Date.new(2026, 1, 2), Date.new(2026, 1, 3)].each_with_index do |date, i|
        assert_enqueued_with(job: ScrapeCfWorkoutJob, args: [date],
                             at: (i * BackfillCrossfitWorkoutsJob::DELAY_BETWEEN_REQUESTS).from_now)
      end
    end
  end
  ...
  test 'raises ArgumentError for a reversed range, enqueueing nothing' do
    assert_no_enqueued_jobs do
      assert_raises(ArgumentError) do
        BackfillCrossfitWorkoutsJob.perform_now(Date.new(2026, 1, 3), Date.new(2026, 1, 1))
      end
    end
  end
  ...
  test 'a 3-day range imports the right count idempotently, isolating a mid-range failure' do
    ...
    perform_enqueued_jobs { BackfillCrossfitWorkoutsJob.perform_later(Date.new(2018, 1, 10), Date.new(2018, 1, 12)) }
