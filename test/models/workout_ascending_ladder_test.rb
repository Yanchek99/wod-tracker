...
class WorkoutAscendingLadderTest < ActiveSupport::TestCase
  ...
  test 'a ladder workout is not treated as a fixed-rep amrap' do
    ...
    workout.exercises.build(movement: movements(:thruster), position: 1, reps: 3)
  ...
  test 'an exercise rides the ladder from its own reps as the round-1 start' do
    exercise = ladder_workout.exercises.build(movement: movements(:thruster), position: 1, reps: 3)
  ...
  test 'ladder_step_every holds the reps for several rounds before incrementing' do
    exercise = ladder_workout.exercises.build(movement: movements(:thruster), position: 1, reps: 3, ladder_step_every: 3)
  ...
  test 'a ladder-exempt exercise stays constant' do
    exercise = ladder_workout.exercises.build(movement: movements(:run), position: 1, reps: 10, ladder_exempt: true)
  ...
  test 'the ladder step is part of the content fingerprint' do
    ...
    by_three.exercises.build(movement: movements(:thruster), position: 1, reps: 3)
    by_six = ladder_workout(step: 6)
    by_six.exercises.build(movement: movements(:thruster), position: 1, reps: 3)
