...
module CfWorkout
  class FetcherTest < ActiveSupport::TestCase
    ...
    test 'fetches a modern-template workout reached via redirect, re-requesting with a fresh cache-buster' do
      stub_cf_workout_redirect('2025/06/18', '250618')
      stub_request(:get, %r{\Ahttps://www\.crossfit\.com/250618}).to_return(status: 200, body: cf_workout_fixture('modern_normal.html'))
    ...
    test 'fetches a modern-template multi-part workout, with scaling detected despite no "Scaling" heading' do
      ...
      stub_request(:get, %r{\Ahttps://www\.crossfit\.com/260620}).to_return(status: 200, body: cf_workout_fixture('modern_multi_part.html'))
    ...
    test 'retries when the modern route server-renders a content-less shell' do
      ...
      stub_request(:get, %r{\Ahttps://www\.crossfit\.com/260620}).to_return(
        ...
        { status: 200, body: cf_workout_fixture('modern_multi_part.html') }
      )
    ...
    test 'detects a modern-template rest day' do
      ...
      stub_request(:get, %r{\Ahttps://www\.crossfit\.com/260702}).to_return(status: 200, body: cf_workout_fixture('modern_rest_day.html'))
    ...
    test 'fetches a legacy-template workout with inline scaling' do
      stub_request(:get, %r{\Ahttps://www\.crossfit\.com/workout/2018/01/10}).to_return(status: 200, body: cf_workout_fixture('legacy_with_scaling.html'))
    ...
    test 'raises FetchError when redirects exceed the max' do
      stub_cf_workout_redirect('2030/01/03', 'loop')
