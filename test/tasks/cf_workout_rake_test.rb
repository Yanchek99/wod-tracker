...
class CfWorkoutRakeTest < ActiveSupport::TestCase
  setup do
    Rails.application.load_tasks if Rake::Task.tasks.empty?
    Rake::Task['cf_workout:fetch'].reenable
    Rake::Task['cf_workout:scrape'].reenable
    Rake::Task['cf_workout:backfill'].reenable
  end
  ...
  test 'fetches, parses, and prints a workout for a given date' do
    stub_request(:get, %r{\Ahttps://www\.crossfit\.com/workout/2018/01/10})
      .to_return(status: 200, body: Rails.root.join('test/fixtures/cf_workout/legacy_with_scaling.html').read)

    output = capture_io { Rake::Task['cf_workout:fetch'].invoke('2018-01-10') }.join
  ...
  test 'aborts with a usage message when no date is given' do
    error = assert_raises(SystemExit) { Rake::Task['cf_workout:fetch'].invoke }
  ...
  test 'aborts with the fetch error message on a failed fetch' do
    ...
    error = assert_raises(SystemExit) { Rake::Task['cf_workout:fetch'].invoke('2030-01-01') }
  ...
  test 'aborts with the parse error message on an unparseable workout' do
    ...
    stub_request(:get, %r{\Ahttps://www\.crossfit\.com/260620})
      .to_return(status: 200, body: Rails.root.join('test/fixtures/cf_workout/modern_multi_part.html').read)

    error = assert_raises(SystemExit) { Rake::Task['cf_workout:fetch'].invoke('2026-06-20') }
  ...
  test 'scrape persists a Workout and Schedule for a given date' do
    ...
    output = capture_io { Rake::Task['cf_workout:scrape'].invoke('2018-01-10') }.join
  ...
  test 'scrape aborts with a usage message when no date is given' do
    error = assert_raises(SystemExit) { Rake::Task['cf_workout:scrape'].invoke }
  ...
  test 'scrape aborts with the WorkoutImport failure message on an unparseable workout' do
    ...
    error = assert_raises(SystemExit) { Rake::Task['cf_workout:scrape'].invoke('2026-06-20') }

    assert_equal 1, error.status
  end

  test 'backfill enqueues ScrapeCfWorkoutJob for each date in the range and reports the count' do
    output = capture_io { Rake::Task['cf_workout:backfill'].invoke('2026-01-01', '2026-01-03') }.join
  ...
  test 'backfill aborts with a usage message when a date is missing' do
    error = assert_raises(SystemExit) { Rake::Task['cf_workout:backfill'].invoke('2026-01-01') }
  ...
  test 'backfill aborts with the range error message when start is after end' do
    error = assert_raises(SystemExit) { Rake::Task['cf_workout:backfill'].invoke('2026-01-03', '2026-01-01') }
